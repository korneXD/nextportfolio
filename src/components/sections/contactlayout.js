...
export default function ContactLayout() {
    ...
    return (
        <section className="flex w-full min-h-screen flex-col justify-center items-center px-28 bg-black pb-30">
            <h2 data-scroll data-scroll-speed="0.15" className="text-white relative text-[3vw] uppercase">Contact</h2>
            <div className="flex flex-col relative justify-center items-center w-full">
                {
                    contact.map((e) => (
                        <AnimatedText key={e.name} >
                            <span className=' mt-[40px] mb-[20px] whitespace-nowrap cursor-pointer border-b border-gray-200 w-full text-left'>{e.name} - <Link href={e.to} target="_blank">{e.con}</Link>
                            </span>
                        </AnimatedText>)
                    )}
            </div>
